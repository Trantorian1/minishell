...
static size_t	tokenise_redir(
	...
	if (redir == NULL || i_curr + 1 >= input.len)
		return (input.len);

	len = 1 + (input.get[i_curr] == input.get[i_curr + 1]);
	if (input.get[len + 1] == '>' || input.get[len + 1] == '<')
		return (input.len);

	vstr_append(redir, str_substr(input, i_curr, i_curr + len));

	i_curr = skip_whitespace(input, i_curr + len);
	i_prev = i_curr;
	...
	while (
		...
		if (input.get[i_curr] == '\'')
		{
			...
			i_curr = tokenise_quote(redir, input, i_curr, '\'');
			i_prev = i_curr;
		}
...
static uint8_t	tokenise_arg(
	...
	while (i_curr < input.len)
	{
		if (input.get[i_curr] == '\'')
		{
			...
			i_curr = tokenise_quote(arg, input, i_curr, '\'');
			i_prev = i_curr;
		}
		else if (input.get[i_curr] == '"')
		{
		...
		{
			save_prev(arg, input, i_prev, i_curr);
			i_curr = tokenise_redir(redir, input, i_curr);
			i_prev = i_curr;
		}
		else if (is_whitespace(input.get[i_curr]))
		{
